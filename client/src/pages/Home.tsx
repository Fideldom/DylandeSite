...
export default function Home() {
  ...
  useEffect(() => { if (reduce) return; const timer = window.setInterval(() => setActive(i => (i + 1) % slides.length), 6500); return () => window.clearInterval(timer); }, [reduce]);
  const slide = slides[active];
  return <div className="home-page">
    <section className="hero-new"><motion.div className="hero-new-media" key={slide.image} style={{ backgroundImage: `url(${slide.image})` }} initial={{ opacity: 0, scale: 1.045 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1.15, ease: [0.22, 1, .36, 1] }} /><div className="hero-new-shade" /><div className="container hero-new-inner"><div className="hero-new-copy"><span className="eyebrow hero-eyebrow"><i />{slide.eyebrow}</span><motion.h1 key={slide.accent} initial={{ opacity: 0, y: 28, clipPath: "inset(100% 0 0 0)" }} animate={{ opacity: 1, y: 0, clipPath: "inset(0% 0 0 0)" }} transition={{ duration: .9, ease: [0.22, 1, .36, 1] }}>O negócio cresce quando a operação ganha <em>{slide.accent}</em></motion.h1><p>A DYLANDE une software empresarial, consultoria e suporte próximo para transformar complexidade em decisões simples.</p><div className="hero-new-actions"><Link href="/software" className="button button-primary">Conhecer os softwares <ArrowUpRight size={16} /></Link><Link href="/contacto" className="button button-quiet">Falar com um especialista <ArrowRight size={16} /></Link></div><div className="trust-line"><span><Check size={14} /> Software próprio</span><span><Check size={14} /> Suporte local</span><span><Check size={14} /> Certificação AGT</span></div></div><div className="hero-new-side"><span>DY / 2026</span><span>Operação em movimento</span><div className="hero-pager">{slides.map((s, i) => <button key={s.image} aria-label={`Ver apresentação ${i + 1}`} className={i === active ? "active" : ""} onClick={() => setActive(i)}><b>0{i + 1}</b><i /></button>)}</div></div></div></section>
    <Reveal><section className="proof-bar"><div className="container proof-bar-grid"><div><span>01</span><b>Software</b><small>feito para trabalhar</small></div><div><span>02</span><b>Serviço</b><small>que não desaparece</small></div><div><span>03</span><b>Confiança</b><small>que se documenta</small></div><div><span>04</span><b>Escala</b><small>que começa no essencial</small></div></div></section></Reveal>
